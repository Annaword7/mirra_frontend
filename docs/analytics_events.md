...
**Удаление аккаунта.** Клиент шлёт `analytics_id` бэкенду, тот после успешного
`delete_user_cascade` вызывает Amplitude User Privacy API (EU-хост, пара
`AMPLITUDE_API_KEY`/`AMPLITUDE_SECRET_KEY` в env бэкенда; удаление исполняется
до 30 дней). Клиент затем делает `reset()` и чеканит новую идентичность —
