...
Источник схемы — таблица маркетинга «Mirra Marketing - Events»: 85 событий в
исходной версии и ещё 6, добавленных позже (скрыть, открыть, удалить продукт и
вкладка «Рутина»). Отправляется 87 из 91; четыре не отправляются: под два нет
экрана, два свёрнуты в `analysis_failed` — см. «Что изменилось против
таблицы». Всего в коде 108 событий: 87 из таблицы и 21 сверх неё (18 живших до
таблицы, `purchase_restore`, `routine_view_analysis` и
`onboarding_skin_continue`).
...
`backendhost`. У dev и prod **разные проекты Amplitude**, чтобы тестовые прогоны
не смешивались с живыми пользователями:

| Сборка | Файл | Проект Amplitude |
|---|---|---|
| prod | `environment.json` | прод-проект, `e73d73d5…` |
| dev (`APP_ENV=dev`) | `environment.dev.json` | дев-проект, `e231b8b6…` |
...
В приложение попадает только API key: он умеет лишь отправлять события и
публичен по замыслу. Secret key для Deletion API живёт только на бэкенде
(`AMPLITUDE_API_KEY` + `AMPLITUDE_SECRET_KEY` в env), у каждого окружения пара
своего проекта. Оба проекта должны быть в одной организации: зона EU зашита в
коде.
...
`APP_ENV` по умолчанию `prod`, но `ios/Flutter/Generated.xcconfig` кэширует
дефайн последней сборки: после дев-сборки и Xcode Run будет dev, и события уйдут
в дев-проект. Задавайте `--dart-define=APP_ENV=…` явно и смотрите, в какой из
двух проектов пришли события.
...
| `onboarding_skin_continue` | всё событие | «Далее» на шаге «Как кожа себя ведёт?» в таблице не было: завершение второго шага воронки приходилось выводить по событиям третьего. |
...
### RevenueCat → Amplitude

Интеграция в RevenueCat шлёт серверные события подписки (покупка, продление,
истечение, возврат) напрямую в Amplitude, с выручкой. Кем считать событие,
RevenueCat решает по атрибутам подписчика, их ставит `revenue_cat.login()`:

- `$amplitudeDeviceId` — Keychain-идентичность устройства, всем;
- `$amplitudeUserId` — Supabase uuid, только аккаунту.

Аноним получает только device_id: если задан лишь он, RevenueCat не шлёт
user_id, и покупка ложится в анонимную историю устройства, не назначая ей
user_id (иначе анонимный uuid стал бы user_id через чёрный ход, и вход через
Apple после покупки разрывал бы историю). Без атрибутов RevenueCat слал бы
app_user_id, то есть анонимный Supabase uuid, и каждая анонимная покупка
повисала бы отдельным пользователем.

В настройках интеграции: регион EU, API key прод-проекта, Sandbox API key
dev-проекта (песочные покупки, включая TestFlight, уходят в dev). События
названы с префиксом `rc_` (`rc_initial_purchase`, `rc_renewal`,
`rc_expiration` …), чтобы не путать с `purchase_completed`, которое шлёт
приложение.
...
| `onboarding_skin_continue` | `type_new`, `type_eruption` | «Далее» на шаге «Как кожа себя ведёт?» — добавлено сверх таблицы |
...
| `product_hidden` | — | «Скрыть» в меню; скан уходит из общего каталога |
| `product_hidden_ok` | — | «Хорошо» в окне «Продукт скрыт» |
| `product_public_catalog` | — | «Открыть» в меню; скан возвращается в каталог |
| `product_public_catalog_ok` | — | «Хорошо» в окне «Продукт в общем каталоге» |
| `product_delete` | — | «Удалить» в меню, до окна подтверждения |
...
| `anon_session_started` | — — создана гостевая сессия: первый запуск, после выхода, после удаления аккаунта. Уходит из `_signInAnonymously` в auth-менеджере, а не из листа быстрой настройки, где раньше почти не срабатывало |
| `anon_converted` | `method`: `email`, `apple` — гость стал **новым** аккаунтом. Вход гостя в существующий аккаунт не считается: это возвращение, а не конверсия. По email уходит из `_linkEmailToAnonymousAccount`, по Apple только если `created_at` аккаунта совпадает с моментом входа |
