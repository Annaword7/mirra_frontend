...
void main() {
  ...
  test('ссылка на Storage переписывается в уменьшенную версию', () {
    expect(
      ...
      '?width=400&resize=contain&quality=75',
    );
  });

  test('качество настраивается', () {
    expect(thumbUrl(storageUrl, width: 200, quality: 60),
        endsWith('?width=200&resize=contain&quality=60'));
  });

  test('существующая query-строка сохраняется', () {
    expect(
      thumbUrl('$storageUrl?token=abc', width: 200),
      endsWith('1.png?token=abc&width=200&resize=contain&quality=75'),
    );
  });

  test('resize=contain обязателен: без него Storage режет по cover', () {
    // С одним width Storage вписывает в «width × высота оригинала» и отдаёт
    // вертикальную полосу из середины кадра — так ломались лента и Косметичка.
    expect(thumbUrl(storageUrl, width: 720), contains('resize=contain'));
  });
